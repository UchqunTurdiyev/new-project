...
export const Hero = ({ trending }: HerpProps): JSX.Element => {
	...
	const handleCurrentModal = () => {
		setModal(false), setCurrentMovie(movie);
	};
	...
	return (
		<div className='flex flex-col space-y-2 pt-28 pb-10 md:space-y-4 lg:h-[65vh] lg:pb-12 lg:justify-center'>
			...
			<div className='w-[100px] rounded-bl-xl text-lg rounded-tr-xl text-center px-1 py-[2px] bg-black/50 text-white'>
				{movie.media_type}
			</div>
			...
			<p className='max-w-xs max-lg:max-w-xl max-sm:w-5/6 mg:max-w-lg lg:max-w-2xl text-xs md:text-lg lg:text-2xl lg:leading-10'>
				{movie.overview?.slice(0, 140)}...
			</p>
			<div className=''>
				<button
					onClick={handleCurrentModal}
					className='flex bg-[#00000080] hover:bg-[#000000ba] transition-all items-center justify-center border-2 text-white font-bold rounded-full max-md:text-sm w-36 h-12 lg:w-52 lg:h-14 '
				>
					<BsPlay className='w-5 h-5 md:h-8 md:w-8' /> Watch now
				</button>
			</div>
		</div>
	);
